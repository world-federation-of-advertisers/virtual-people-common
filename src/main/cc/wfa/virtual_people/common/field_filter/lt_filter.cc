...
absl::StatusOr<std::unique_ptr<LtFilter>> LtFilter::New(
    const google::protobuf::Descriptor* descriptor,
    const FieldFilterProto& config) {
  if (config.op() != FieldFilterProto::LT) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Op must be LT. Input FieldFilterProto: ", config.DebugString()));
  }
  if (!config.has_name()) {
    return absl::InvalidArgumentError(absl::StrCat(
  ...
  return absl::make_unique<LtFilter>(std::move(comparator));
}

bool LtFilter::IsMatch(const google::protobuf::Message& message) const {
  return comparator_->Compare(message) == IntegerCompareResult::LESS_THAN;
}
