...
/**
 * The implementation of [FieldFilter] when op is GT in config.
 *
 * Always use [FieldFilter.create]. Users should never construct a [GtFilter] directly.
 */
internal class GtFilter(descriptor: Descriptors.Descriptor, config: FieldFilterProto) :
  FieldFilter {
  ...
  init {
    if (config.op != FieldFilterProto.Op.GT) {
      error("Op must be GT. Input FieldFilterProto: $config")
    }
    if (!config.hasName()) {
