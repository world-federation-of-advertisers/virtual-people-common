...
/** Implementation of AnyFilter for [ValueType]. */
internal class AnyInFilterImpl<ValueType>(
  fieldDescriptors: List<FieldDescriptor>,
  private val parsedValues: Set<ValueType>
  ...
  override fun matches(messageOrBuilder: MessageOrBuilder): Boolean {
    ...
    values.forEach {
      ...
      if (parsedValues.contains(it as ValueType)) {
        return true
      }
    }
    return false
